...
export default function PesanPelangganPage() {
  ...
  // Ambil daftar menu dari localStorage yang sama dengan kasir dan manajemen menu
  useEffect(() => {
  ...
  // Fungsi Kirim Pesanan ke Kasir, Dashboard, & Update Status Meja Otomatis
  const kirimPesanan = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    // 2. OTOMATIS UPDATE STATUS MEJA KE 'Terisi' BERDASARKAN NOMOR MEJA
    const matchNomor = nomorMeja.match(/\d+/);
    if (matchNomor) {
      const mejaId = parseInt(matchNomor[0], 10);
      
      const savedMeja = localStorage.getItem('rjresto_meja');
      let mejaList = savedMeja ? JSON.parse(savedMeja) : [
        { id: 1, status: 'Kosong' }, { id: 2, status: 'Kosong' },
        { id: 3, status: 'Kosong' }, { id: 4, status: 'Kosong' },
        { id: 5, status: 'Kosong' }, { id: 6, status: 'Kosong' },
      ];

      mejaList = mejaList.map((m: any) => 
        m.id === mejaId ? { ...m, status: 'Terisi' } : m
      );

      localStorage.setItem('rjresto_meja', JSON.stringify(mejaList));
    }
